...
class QMessage(object):
    ...
    def __str__(self, *args, **kwargs):
        return (f'QMessage: message type: {self._type}, data size: {self._size}, '
                f'is_compressed: {self._is_compressed}, data: {self._data}')
...
class QReader(object):
    ...
    def read_data(self, message_size, is_compressed=False, **options):
        ...
        if is_compressed:
            ...
            raw_data = uncompress(raw_data, np.intc(uncompressed_size))
            raw_data = np.ndarray.tostring(raw_data)
    ...
    def _read_symbol(self, qtype=QSYMBOL):
        return np.bytes_(self._buffer.get_symbol())
    ...
    def _read_list(self, qtype):
        ...
        if qtype == QSYMBOL_LIST:
            ...
            data = np.array(symbols, dtype=np.bytes_)
