...
class PandasQReader(QReader):
        ...
        if self._options.pandas:
            ...
            if isinstance(keys, pd.DataFrame):
                if not isinstance(values, pd.DataFrame):
                    raise QReaderException(f'Keyed table creation: values are expected to be of '
                                           f'type pandas.DataFrame. Actual: {type(values)}')
                ...
                keys = keys if not isinstance(keys, pd.Series) else keys.values
                values = values if not isinstance(values, pd.Series) else values.values
    ...
    def _read_table(self, qtype=QTABLE):
        if self._options.pandas:
            ...
            for i in range(len(columns)):
                column_name = columns[i] if isinstance(columns[i], str) else columns[i].decode(
                    "utf-8")
                if isinstance(data[i], str):
                    # convert character list (represented as string) to numpy representation
                    meta[column_name] = QSTRING
                    odict[column_name] = pd.Series(list(data[i]), dtype=np.str).replace(b' ', np.nan)
                elif isinstance(data[i], bytes):
                    # convert character list (represented as string) to numpy representation
                    meta[column_name] = QSTRING
                    odict[column_name] = pd.Series(list(data[i].decode()), dtype=np.str).replace(b' ', np.nan)
                elif isinstance(data[i], (list, tuple)):
                    meta[column_name] = QGENERAL_LIST
                    tarray = np.ndarray(shape=len(data[i]), dtype=np.dtype('O'))
    ...
    def _read_list(self, qtype):
        ...
        if self._options.pandas:
            if -abs(qtype) not in [QMONTH, QDATE, QDATETIME, QMINUTE, QSECOND,
                                   QTIME, QTIMESTAMP, QTIMESPAN, QSYMBOL]:
                null = QNULLMAP[-abs(qtype)][1]
                ps = pd.Series(data=qlist).replace(null, np.nan)
            else:
                ps = pd.Series(data=qlist)
    ...
    def _read_general_list(self, qtype=QGENERAL_LIST):
        ...
        if self._options.pandas:
            return [np.nan if isinstance(element, basestring)
...
class PandasQWriter(QWriter):
    ...
    def _write_pandas_series(self, data, qtype=None):
        ...
        if qtype is None and data.dtype.type in (np.datetime64, np.timedelta64):
        ...
        if qtype == QGENERAL_LIST:
            ...
            self._write_string(data.replace(np.nan, ' ').values.astype(np.string_).tostring())
        elif data.dtype.type not in (np.datetime64, np.timedelta64):
    ...
    def _write_pandas_data_frame(self, data, qtype=None):
        ...
        if hasattr(data, 'meta') and data.meta.qtype == QKEYED_TABLE:
            ...
            self._write(qlist(np.array(index_columns), qtype=QSYMBOL_LIST))
            data.reset_index(inplace=True)
            self._buffer.write(struct.pack('=bxi', QGENERAL_LIST, len(index_columns)))
            for column in index_columns:
                self._write_pandas_series(data[column], qtype=data.meta[column]
                                          if hasattr(data, 'meta') else None)
            data.set_index(index_columns, inplace=True)

        self._buffer.write(struct.pack('=bxb', QTABLE, QDICTIONARY))
        self._write(qlist(np.array(data_columns), qtype=QSYMBOL_LIST))
        self._buffer.write(struct.pack('=bxi', QGENERAL_LIST, len(data_columns)))
        for column in data_columns:
            self._write_pandas_series(data[column],
    ...
    def _write_generic_list(self, data):
        ...
        for element in data:
            ...
            self._write(' ' if type(element) in [float, np.float32, np.float64] and np.isnan(
